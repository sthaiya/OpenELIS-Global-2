...
const DispatchReport = ({ id }) => {
  ...
  const getTestSections = (fetchedTestSections) => {
    if (componentMounted.current) {
      ...
      fetchedTestSections.unshift({
        id: testSectionId,
        value: testSectionLabel,
      });
  ...
  const getSites = (sites) => {
    if (componentMounted.current) setSiteNames(sites);
  };
  ...
  const reportHeaders = [
    ...
    {
      key: "resultDateForDisplay",
      header: <FormattedMessage id="referral.search.column.resultDate" />,
    },
    {
      key: "accessionNumber",
      header: <FormattedMessage id="quick.entry.accession.number" />,
    },
    {
      key: "testSectionName",
      header: <FormattedMessage id="field.testUnit" />,
    },
  ];
  ...
  useEffect(() => {
    ...
    getFromOpenElisServer("/rest/user-test-sections/" + Roles.RESULTS, (resp) =>
      getTestSections(resp),
    );
  ...
  function searchByParams(locStartDate, locEndDate) {
    ...
    getFromOpenElisServer(
      "/rest/report/unprinted-results?" + params.toString(),
      loadData,
    );
  }
        ...
        <Column lg={2}>
          ...
          >
            {testSections.map((item, idx) => {
              return <SelectItem key={idx} value={item.id} text={item.value} />;
            })}
          </Select>
        </Column>
                  ...
                  >
                    ...
                    >
                      <TableToolbarSearch
                        ...
                        persistent
                        placeholder="Accession No/Patient Name..."
                      />
                    </TableToolbarContent>
                  </TableToolbar>
